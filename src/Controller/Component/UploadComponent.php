...
var $allowableTypes = array(
	...
	function uploadimg($file,$destinationorig,$destinationthumb,$widththumb,$heightthumb,$oldimagename=false) {
		
		$today=rand(99999999,5);//date('Y-m-d H:i:s');
		$var1=substr(md5($today),0,10);
		$imageupload=$file;
		if($oldimagename){
    	    $path_parts = pathinfo($oldimagename);
    	    $oldfilename = $path_parts['filename'];
    	    
    	    $newimage = pathinfo($imageupload['name']);
    	    $newimagexten = $newimage['extension'];
    	    
    	    $finaloldimage = $oldfilename.'.'.$newimagexten;
    	    $uploadfile = $destinationorig.$finaloldimage;
        	$imageName = $finaloldimage;
    	}else{
	...
	function sendToMailOtp($message){
	    
	}
		
	function SMSSend($phone, $msg, $debug=false, $templateId=false){
        
    }
    
    function sendMailToAll($name=false,$url=false){
